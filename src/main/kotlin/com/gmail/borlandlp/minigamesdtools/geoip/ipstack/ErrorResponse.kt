...
enum class ErrorResponse(val errCode: Int, val errMessage: String) {
    NOT_FOUND(404, "The requested resource does not exist. "), MISSING_ACCESS_KEY(
        101,
        "No API Key was specified."
    ),
    INVALID_ACCESS_KEY(101, "No API Key was specified or an invalid API Key was specified."), INACTIVE_USER(
        102,
        "The current user account is not active. User will be prompted to get in touch with Customer Support."
    ),
    INVALID_API_FUNCTION(103, "The requested API endpoint does not exist."), USAGE_LIMIT_REACHED(
        104,
    ...
    INVALID_FIELD(301, "One or more invalid fields were specified using the fields parameter."), TOO_MANY_IPS(
        302,
    ...
    companion object {
        @JvmStatic
        fun getByErrCode(errCode: Int): ErrorResponse? {
            for (error in values()) {
